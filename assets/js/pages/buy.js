...
function loadCars() {
  fetch('../../assets/data/cars-100.json')
    ...
    .catch(error => {
      console.error('cars-100.json 로드 실패:', error);
      cars = [];
      filteredCars = [];
      if (carGrid) {
        carGrid.innerHTML = '<p class="no-result">차량 데이터를 불러오지 못했습니다.</p>';
      }
      if (carCount) carCount.textContent = '0';
      const mobileTargets = [
        document.querySelector('#mobilePickGrid'),
        document.querySelector('#mobileKiaGrid'),
        document.querySelector('#mobilePopularGrid')
      ];
      mobileTargets.forEach(target => {
        if (target) {
          target.innerHTML = '<p class="mobile-load-error">차량 데이터를 불러오지 못했습니다.</p>';
        }
      });
    });
}
...
function renderCars() {
  ...
  <div class="car-card-image" data-id="${car.id}">
    ...
    >
    <button type="button" class="heart-btn" data-id="${car.id}" aria-label="관심차량" >
  <img src="../../assets/img/buy-img/heart-off.png" alt="찜하기" class="heart-icon">
  </button>
    <!-- 외부 / 내부 -->
...
/* ==================================================
   모바일 차량 렌더링
================================================== */
function renderMobileTag(tag) {
  const text = String(tag || '').trim();
  const isSeat = text.includes('시트');
  if (!isSeat) {
    return `<span>${text}</span>`;
  }
  const seatColor = text.replace(/\s*시트\s*$/, '').trim();
  return `
    <span class="seat-tag">
      <span
        class="seat-color"
...
function renderMobileCars() {
    ...
    <article class="mobile-car-card">
      <a href="../Car%20Detail%20Page/cardetailpage.html?id=${car.id}">
        <div class="mobile-car-image">
          <img src="${car.thumbnail}" alt="${car.modelName}">
          <button type="button" class="mobile-heart" data-id="${car.id}" aria-label="관심차량">
  <img
    src="../../assets/img/buy-img/하트.png"
    alt="찜하기"
  ...
  >
        ...
        <h3>${car.modelName}</h3>
        <p>${String(car.regDate).slice(2, 7).replace('-', '/')}년식 (${String(car.modelYear).slice(2)}년형)<br>${numberFormat(car.mileage)}km</p>
        <strong>${numberFormat(car.price)}<em>만원</em></strong>
        <small>60개월시 월 ${numberFormat(car.monthlyPayment)}원</small>
        <div class="mobile-tags">
          ${(car.tags || []).slice(0, 2).map(renderMobileTag).join('')}
        </div>
      </a>
    </article>`;
  const makePopularCard = car => `
    <article class="mobile-popular-card">
      <a href="../Car%20Detail%20Page/cardetailpage.html?id=${car.id}">
        <div class="mobile-popular-image">
          <img src="${car.thumbnail}" alt="${car.modelName}">
          <button type="button" class="mobile-heart" data-id="${car.id}" aria-label="관심차량">
  <img
    src="../../assets/img/buy-img/하트.png"
  ...
  >
        ...
        <h3>${car.modelName}</h3>
        <p>${String(car.regDate).slice(2, 7).replace('-', '/')}년식 (${String(car.modelYear).slice(2)}년형) ${numberFormat(car.mileage)}km ${car.region || ''}</p>
        <strong>${numberFormat(car.price)}<em>만원</em></strong>
        <small>60개월시 월 ${numberFormat(car.monthlyPayment)}원</small>
        <div class="mobile-tags">
          ${(car.tags || []).slice(0, 3).map(renderMobileTag).join('')}
        </div>
      </a>
    </article>`;
  ...
  document.querySelectorAll('.mobile-heart').forEach(button => {
    button.addEventListener('click', event => {
      event.preventDefault();
      event.stopPropagation();
      const icon = button.querySelector('.mobile-heart-icon');
      button.classList.toggle('active');
      if (button.classList.contains('active')) {
        icon.src = '../../assets/img/buy-img/heart-on.png';
      } else {
        icon.src = '../../assets/img/buy-img/heart-off.png';
      }
    });
  });
}
